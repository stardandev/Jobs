...
export default class Headline extends React.Component {
  ...
  deleteResume(e){
    ...
    this.setState({resumeValid : true,formErrors : error, resumeName : '', resumeObj : this.state.resumeObj , resumeUpdate : true, deleteResumeLink: this.state.resumeLink});
  }
            ...
            _this.setState({completeStatus : response.data.Data.job_seeker_info.last_visited_page});
              ...
              if(!!photoUrl){
               ...
               // var photoName = photoLink.substr(photoLink.lastIndexOf('/') + 1);
                ...
                _this.setState({croppedImg : _this.state.photoLink, deleteDisabled : _this.state.deleteDisabled, photoLink: _this.state.photoLink, photoUrl: photoUrl});
              }
...
handleFileChange(dataURI, imgObj) {
  ...
  deleteImage() {
    ...
    error.photo = "";
    this.state.imgObj = {};
    this.setState({photoValid : true,formErrors : error, img: null, croppedImg: "/images/upload_image_placeholder.png", imgObj : this.state.imgObj , photoUpdate : true , deleteDisabled : true, deletePhotoLink: this.state.photoUrl});
  }
    ...
    return (
      <Role step="headline" role="seeker" profileStatus={this.state.completeStatus}>
        <div className="visible-xs mobile-page-heading">
          <span className="previous" onClick={() => utils.changeUrl(routesPath['SEEKER_EXEPERIENCE'])}></span> Headline + Additional Info <span className={this.state.completeStatus >= 3 ? 'next' : 'next disabled-element'} onClick={() => utils.changeUrl(routesPath['SEEKER_JOB_TYPE'])}></span>
        </div>
        <div className="network-form form">
          <div className="network-card card">
            ...
            <div className="upload-photo">
              ...
              <span className={this.state.deleteDisabled ? 'd-none' : "d-block"} data-toggle="modal" onClick={this.onImageCrossIconClick}>
            ...
            <div className="row">
              <div className={this.state.formErrors.resume !== '' ? 'col-sm-6 upload-document global-error' : 'col-sm-6 upload-document'}>
                <div className="dropzone">
                  <Dropzone accept="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onDrop={this.onDropFile} maxSize={this.state.maxSize} multiple={this.state.multiple} />
                  <div className={(this.state.resumeName != '' ) ? "m-0 form-group row d-none" : "m-0 form-group row d-block"}>
                    <span className="d-inline-block">
                      <img src={constant['IMG_PATH'] + 'upload-doc.png'} alt="upload-doc" className="img-responsive pull-right" />
                    </span>
                ...
                <p className="">
                  <span className="m-0">{this.state.formErrors.resume  !== '' ? this.state.formErrors.resume : ''}</span>
                </p>
              ...
              <div className={this.state.formErrors.writing_samples !== '' ? 'col-sm-6 upload-document global-error' : 'col-sm-6 upload-document'}>
                <div className="dropzone">
                  <Dropzone accept="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onDrop={this.onDrop} maxSize={this.state.maxSize} />
                  ...
                  <aside className={(this.state.alreadyAccepted.length > 0 || this.state.accepted.length > 0 || this.state.rejected.length > 0) ? "d-block" : "d-none"}>
                    <div className={this.state.expand ? "file-wrapper" : "file-wrapper max-height-255"}>
                      <ul>{ this.state.alreadyAccepted.map((f,index) => <li key={index}> <i className="fa fa-file-text-o" aria-hidden="true"></i> <span className="trunc" title={f.name}>{f.name}</span> <span onClick={(e) => this.deleteFile('alreadyAccepted',index)}> <i className="fa fa-trash-o" aria-hidden="true"></i></span></li>) }</ul>
                      <ul>{ this.state.accepted.map((f,index) => <li key={index}> <i className="fa fa-file-text-o" aria-hidden="true"></i> <span className="trunc" title={f.name}>{f.name}</span> <span onClick={(e) => this.deleteFile('accepted',index)}> <i className="fa fa-trash-o" aria-hidden="true"></i></span></li>) }</ul>
                      <ul>{ this.state.rejected.map((f,index) => <li key={index}> <i className="fa fa-file-text-o" aria-hidden="true"></i> <span className="trunc" title={f.name}>{f.name}</span> <span onClick={(e) => this.deleteFile('rejected',index)}> <i className="fa fa-trash-o" aria-hidden="true"></i> </span><p><span>{f.error}</span></p></li>) }</ul>
                    </div>
                ...
                <p>
                  <span className="d-none m-0">{this.state.formErrors.writing_samples  !== '' ? this.state.formErrors.writing_samples : ''}</span>
                </p>
          ...
          <div className="nxt-prev-btns">
            <button type="button" onClick={() => utils.changeUrl(routesPath['SEEKER_EXEPERIENCE'])} className="previouse-btn btn pull-left"> Previous </button>
            <div className={this.state.editProfile == true ? "d-block" : "d-none"}>
              <button type="button" name="save&Next" className="nxt-btn btn-primary btn pull-right" onClick={this._handleClick}> Save & Next </button >
              <button type="button" name="save" className="nxt-btn btn-primary btn pull-right mr-1p" onClick={this._handleClick}> Save </button >
              <button type="button" className="nxt-btn btn-default btn pull-right mr-1p" onClick={() => utils.refreshPage()}> Cancel </button>
            </div>
            <button type="submit" name="save&Next" onClick={this._handleClick} className={this.state.editProfile == true ? "d-none nxt-btn btn pull-right" : "d-block nxt-btn btn-primary btn pull-right"}> Next </button >
